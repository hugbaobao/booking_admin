...
const addBanner = async (files) => {
  const res = await Api.bannerBtmAddApi(files) as any
  if(res.code === 200) {
...
const getBannertop = async () => {
  const { data: res } = await Api.bannerBtmGetApi() as any
  state.value = res.clockbtm
  bannertop.value = Object.values(res.bannerbtm)
}

// 修改page
const changePage = async (list) => {
  const res = await Api.bannerBtmpageApi(list) as any
  if(res.code === 200) {
...
const updateBanner = async (datas) => {
  const res = await Api.bannerBtmimgApi(datas) as any
  if(res.code === 200) {
    message.success('修改成功');
    getBannertop()
  } else {
...
const changeState = async (value: boolean) => {
  const res = await Api.bannerBtmStateApi(value) as any
  if(res.code === 200) {
    message.success('修改成功');
    getBannertop()
  } else {
...
<template>
  ...
  <global-block :setMargin="'0 0 15px'">
    ...
    <template #default>中间轮播图</template>
