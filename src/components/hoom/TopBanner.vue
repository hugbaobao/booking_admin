...
const addBanner = async (files) => {
  const res = await Api.bannerTopAddApi(files) as any
  if(res.code === 200) {
...
const getBannertop = async () => {
  const { data: res } = await Api.bannerTopGetApi() as any
  state.value = res.clocktop
  bannertop.value = Object.values(res.bannertop)
}

// 修改page
const changePage = async (list) => {
  const res = await Api.bannerToppageApi(list) as any
  if(res.code === 200) {
...
const updateBanner = async (datas) => {
  const res = await Api.bannerTopimgApi(datas) as any
  if(res.code === 200) {
    message.success('修改成功');
    getBannertop()
  } else {
...
const changeState = async (value: boolean) => {
  const res = await Api.bannerTopStateApi(value) as any
  if(res.code === 200) {
    message.success('修改成功');
    getBannertop()
  } else {
...
<template>
  ...
  <global-block :setMargin="'0 0 15px'">
    ...
    <template #default>顶部轮播图</template>
